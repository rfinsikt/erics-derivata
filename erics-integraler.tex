...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Det röda områdets area är integralen av $f(x)$ från 1 till 3.}
    \label{fig:1}
\end{figure}

Vi skriver arean av det röda området i figur \ref{fig:1} med symboler så här: 

\[ \int_1^3 f(x)dx \] 

Vi uttalar det som ``integralen från 1 till 3 av f av x dx''. Integraltecknet $\int$ ser ut som ett utdraget S och vi skriver gränser längst upp och längst ner på s:et. Talet nedanför berättar områdets vänsterkant och talet ovanför berättar områdets högerkant. Det lilla tillägget ``dx'' har en djup och intressant matematisk mening som du kan lära dig om nån annan gång --- för nu så nöjer vi oss med att ``dx''-biten berättar att det är tecknet ``x'' som är funktionens variabel.
...
\begin{exempel}
    \label{ex:2x}
    Hitta alla primitiva funktioner till $f(x) = 2x$
    \dinkus
    Svar: $F(x) = x^2 + C$
\end{exempel}
...
Om du undrar varför $x$ inte får vara -1 så kommer vi att ta upp det fallet senare (specifikt i räkneregel \ref{reg:x^-1} på sidan \pageref{reg:x^-1}). För tillfället så låtsas vi som ingenting och har i bakhuvudet att räkneregel \ref{reg:x^n} inte hjälper oss att hitta primitiva funktioner till $\frac 1x$.
...
\begin{uppgifter}
    ...
    \begin{multicols}{3}
        \begin{enumerate}
            \item $f(x) = x^4 - x \\ F(x) \ans$
            \item $g(x) = 2 + x + x^2 \\ G(x) \ans$
            \item $h(x) = x^{27} - 1 \\ H(x) \ans$
        \end{enumerate}
    \end{multicols}
\end{uppgifter}

\subsection{Primitiva funktioner till funktioner med komplicerade exponenter}
...
\begin{exempel}
    ...
    \[F(x) = \dfrac {x^{-1}}{-1} + \dfrac {x^\frac{4}{3}}{\frac{4}{3}} + \dfrac {x^{3,35}}{3,35} + C \]
    ...
    \[F(x) = - x^{-1} + \dfrac {3x^\frac{4}{3}}{4} + \dfrac {x^{3,35}}{3,35} + C\]
\end{exempel}
...
\subsection{Primitiva funktioner till exponentialfunktioner}
...
\subsection{Ett underligt undantag}
...
Hur kommer det sig? Varför då? Var \%\#\& kom den naturliga logaritmen ifrån? Absolutbelopp??? Alla fyra är utmärkta frågor med intressanta och sublima svar som vi inte har någon plats för i det här lilla häftet. För våra syften så räcker det med att acceptera regeln.

\section{Att beräkna integraler}

Nu är det dags att gå tillbaka till vad vi skulle göra från början --- att beräkna arean under en funktions graf (som i figur \ref{fig:1} på sidan \pageref{fig:1}). Vi kommer att ta hjälp av en av matematikens viktigaste räkneregler --- \emph{analysens fundamentalsats} --- som beskriver precis hur vi kan använda primitiva funktioner för att beräkna integraler. Vill du veta mer om hur man kommer fram till den så kan du titta upp teorin om \emph{Riemann-summor}.
...
Högerledet i räkneregel \ref{reg:fund} här ovanför läser man högt som ``F av x \emph{värderat} från a till b''. Det är ett kortfattat sätt att skriva ``skillnaden i funktionen $F$s värde mellan $x=a$ och $x=b$''. Det kan skrivas ut så här:
...
\begin{exempel}
    ...
    \[\int_1^3x^2dx = \left[\dfrac{x^3}3 + C\right]_1^3 = \left(\dfrac{3^3}3 + C \right) - \left(1^3 + C\right) = 27 - 1 = 26\]
%fiiiixaaaaa!!!
    Notera att alla konstanterna $C$ försvann i uträkningen --- det gör de alltid, så vi kommer i framtiden att med gott samvete ignorera konstanten i de primitiva funktionerna när vi värderar integraler, och skriva uträkningen som:
...
\begin{exempel}
    ...
    \[\int_{1}^{5}3x^2 - 2 dx\]
    \dinkus
    \[\int_{1}^{5}3x^2 - 2 dx = \left[x^3 - 2x\right]_1^5 = \left(5^3 - 2\cdot5\right)- \left(1^3 - 2\cdot 1\right) = 125 - 10 - 1 + 2 = 116\]
    Svar: $\int_{1}^{5}3x^2 - 2 dx = 116$
\end{exempel}

Var noga med tecken här när du löser nästa grupp av uppgifter, kom ihåg parenteser runt $F(a)$ och $F(b)$.

\begin{uppgifter}
    Beräkna följande integraler:
    \begin{enumerate}
        \item \[\int_0^1e^x+xdx \ans[27] \]
        \item \[\int_{-2}^2x^3-x^2dx \ans[27]\]
        \item \[\int_2^32^x - x^2 + x dx \ans[25]\]
    \end{enumerate}
\end{uppgifter}

\appendix
\newpage
\section{Facit övningsuppgifter}

\begin{facit}
    \begin{multicols}{3}
        \begin{enumerate}
            \item $F(x) = \dfrac{x^5}5 + C$
            \item $G(x) = x + C$
            \item $H(x) = \dfrac{x^{28}}{28} + C$
        \end{enumerate}
    \end{multicols}
\end{facit}
...
\begin{facit}
    \begin{multicols}{3}
        \begin{enumerate}
            \item $F(x) = \dfrac{x^5}5 - \dfrac{x^2}2 + C$
            \item $G(x) = 2x + \dfrac{x^2}2 + \dfrac{x^3}3 + C$
            \item $H(x) = \dfrac{x^{28}}{28} - x + C$
        \end{enumerate}
    \end{multicols}
\end{facit}
...
\begin{facit}
    \begin{multicols}{3}
        \begin{enumerate}
            \item $F(x) = \dfrac{2x^{\frac 32}}3 + C$
            \item $G(x) = -\dfrac 1{x} + \dfrac1{2x^2} + C$
            \item $H(x) = \dfrac{4x^{1,5}}{1,5} + C$
        \end{enumerate}
    \end{multicols}
\end{facit}
...
\begin{facit}
    \begin{multicols}{3}
        \begin{enumerate}
            \item $F(x) = -\dfrac1{e^x} + C$
            \item $G(x) = \dfrac{10^{2x}}{2\ln(10)} + C$
            \item $H(x) = -\dfrac1{\ln(3)3^{3x}}$
        \end{enumerate}
    \end{multicols}
\end{facit}
...
\begin{facit}
    \begin{multicols}{3}
        \begin{enumerate}
            \item $e-1 \left(\approx1,72\right)$
            \item $\dfrac {16}3 \left(\approx5,33\right)$
            \item $\dfrac 3{8 \ln(2)} \left(\approx0,54\right)$
        \end{enumerate}
    \end{multicols}
\end{facit}
...
\begin{facit}
    \begin{multicols}{3}
        \begin{enumerate}
            \item $e-\frac12 \left(\approx2,22\right)$
            \item $-\dfrac {16}3 \left(\approx-5,33\right)$
            \item $\dfrac 4{\ln(2)} - \dfrac {23}6 \left(\approx1,94\right)$
        \end{enumerate}
    \end{multicols}
\end{facit}
